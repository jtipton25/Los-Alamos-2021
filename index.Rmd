...
- Lack of good statistical software.    
    ...
    - gitHub packages [pollenReconstruction](https://jtipton25.github.io/pollenReconstruction/), [pgR](https://jtipton25.github.io/pgR/), and [BayesMRA](https://jtipton25.github.io/BayesMRA/) in development. 
...
$$
\begin{align*}
\mathbf{y}_i \left( \mathbf{s}, t \right) & = \left( y_{i1} \left( \mathbf{s}, t \right), \ldots, y_{id} \left( \mathbf{s}, t \right) \right)'
\end{align*}
$$
...
$$
\begin{align*}
\operatorname{log} \left( \boldsymbol{\alpha} \left( \mathbf{s}, t \right) \right) & = \mathbf{z}\left( \mathbf{s}, t \right) \boldsymbol{\beta}.
\end{align*}
$$
...
$$
\begin{align*}
\operatorname{log} \left( \boldsymbol{\alpha} \left( \mathbf{s}, t \right) \right) & = \mathbf{B} \left( \mathbf{z}\left( \mathbf{s}, t \right) \right) \boldsymbol{\beta}
\end{align*}
$$

<hr style="height:8px; visibility:hidden;" />

* $\mathbf{B} \left( \mathbf{z}\left( \mathbf{s}, t \right) \right)$ is a basis expansion of the covariates $\mathbf{z}\left( \mathbf{s}, t \right)$.
    * Use B-splines or Gaussian Processes as a basis.
    * $\mathbf{z}\left( \mathbf{s}, t \right)$ is unknown for $t \neq 1$.  
    * Computationally challenging.

<hr style="height:8pxpx; visibility:hidden;" />

<font size="3">
`r capture.output(print(biblio["nolan2019comparing"]))`
...
* Estimate the latent process $\mathbf{z} \left( \mathbf{s}, t \right)$.
...
## Computational Details | Sequential model fitting
...
<font size="3">
`r capture.output(print(biblio["hooten2019prior"]))`
</font>


## Computational Details | Joint Estimation

* Single stage Bayesian model.
...
<font size="3">
`r capture.output(print(biblio["tipton2019predicting"]))`
</font>
...
- Assume the constraint $\color{purple}{(\mathbf{W}_m \bs{\alpha}_m(t))'} \bm{1} = 0$ for all $m$ and $t$
...
* In the two-stage model, the data distribution $[y(\mathbf{s})_i | z(\mathbf{s}_i), \boldsymbol{\theta}_y]$ does not appear in the Metropolis ratio \ref{eq:MH}.
...
A common choice of $\mathbf{R} \left( \boldsymbol{\theta} \right)$ is the Matérn correlation function given by 

$$
\begin{align*}
R( \mathbf{s}_i, \mathbf{s}_j | \boldsymbol{\theta} ) & = \tau^2 \frac {2^{1-\nu }}{\Gamma (\nu )} \left( {\sqrt {2\nu }} \frac{\|\mathbf{s}_i - \mathbf{s}_j \|}{\phi} \right)^{\nu }K_{\nu } \left( {\sqrt {2\nu }} \frac{\|\mathbf{s}_i - \mathbf{s}_j \|}{\phi} \right).
\end{align*}
$$

* $\boldsymbol{\theta} = \left( \nu, \phi \right)'$ where $\nu$ controls the smoothness of the process and $\phi$ controls the spatial range of the correlation.
...
$$
\begin{align*}
\lambda(\mathbf{s}_i) & \sim \operatorname{Gamma} \left( \epsilon_\lambda, \epsilon_\lambda \right).
\end{align*}
$$
...
$$
\begin{align*}
\bar{\mu}_i & = \mathbf{X}_i \boldsymbol{\beta} + \boldsymbol{\Sigma}_{i, -i}  \boldsymbol{\Sigma}^{-1}_{-i, -i} \left( \log \left( \boldsymbol{\lambda} (\mathbf{s}_{-i}) \right) - \mathbf{X}_i \beta \right) \mbox{ and} \\ 
...
## Spatial Poisson Regression - Simulation study

* Two sample sizes for proof of concept.

* Full rank spatial process of 150 (small) and 300 (medium) samples.

    * Note: typically full-rank spatial models are computationally intractable at about 1000 samples.
...
* Hybrid sampler between MH random sample and IS.
    * Similar to [Firefly MCMC](https://arxiv.org/abs/1403.5693).
    
<hr style="height:8px; visibility:hidden;" />

* Density smooth on first stage samples.
    * Allow for a tuneable random walk rather than independent first stage samples.

<hr style="height:8px; visibility:hidden;" />

* Define transient first stage prior using data.
    * Seems like it would violate rules for specifying the prior.
    * Because transient prior is canceled out in second stage, this issue is irrelevant.
...
- Talk available at [www.jtipton25.github.io/Los-alamos-2021](www.jtipton25.github.io/Los-alamos-2021).
